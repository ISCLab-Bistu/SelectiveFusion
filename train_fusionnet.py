...
def main():
	...
	# w_all_list = [[6.0, 3.0]]#skfusion_weight1
	# w_all_list = [[5.0, 0.5]]#skfusion_weight2
	...
	for e in tbar:
		...
		for batch in range(batches):
			...
			for output in outputs:
				...
				# loss_balance_value += 100 * NCC(output, torch.max(x_vi, x_ir))
				# loss_balance_value = 0.001 * ED(output, torch.max(x_vi, x_ir))
				# loss_balance_value += 100 * SCD(output, torch.max(x_vi, x_ir))

				# feature loss
				g2_ir_fea = en_ir#通过主干提取的红外特征
				g2_vi_fea = en_vi#
				g2_fuse_fea = f
