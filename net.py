...
class SKFusion_Block(torch.nn.Module):#融合模块
    def __init__(self, channels, index):
        super(SKFusion_Block, self).__init__()
        ws = [3, 3, 3, 3]
        mid_channel = int(channels / 2)
        self.conv_fusion = ConvLayer(2*channels, channels, ws[index], 1)

        self.conv_ir = ConvLayer(channels, channels, ws[index], 1)
        self.conv_vi = ConvLayer(channels, channels, ws[index], 1)

        self.fc = nn.Linear(channels, mid_channel)
        self.fcs = nn.ModuleList([])
        for i in range(2):
            self.fcs.append(
                nn.Linear(mid_channel, channels)
            )
    ...
    def forward(self, x_ir, x_vi):
        f_cat = torch.cat([x_ir, x_vi], 1)
        f_init = self.conv_fusion(f_cat)#将红外和可见光特征进行融合 conv6
        #先通过两个卷积提取可见光红外光的深度
        out_ir = self.conv_ir(x_ir).unsqueeze(1) #conv1
        out_vi = self.conv_vi(x_vi).unsqueeze(1) #conv2
        #将两者通过相加的方式进行融合
        out_feature = torch.cat([out_ir, out_vi], dim=1)
        fusion_feature = torch.sum(out_feature, dim=1)
        #通过一个全局池化和全连接
        fusion_feature = fusion_feature.mean(-1).mean(-1)#(4,64)
        fusion = self.fc(fusion_feature)#(4,32)
        for i, fc in enumerate(self.fcs):
            vector = fc(fusion).unsqueeze(1)
            if i == 0:
        ...
        out = (out_feature * attention_vectors).sum(dim=1)
...
class Fusion_network(nn.Module):
    def __init__(self, nC, fs_type):
        ...
        #skfusion
        self.fusion_block1 = SKFusion_Block(nC[0], 0)
        self.fusion_block2 = SKFusion_Block(nC[1], 1)
        self.fusion_block3 = SKFusion_Block(nC[2], 2)
        self.fusion_block4 = SKFusion_Block(nC[3], 3)

        #sk_fcafusion
        # self.fusion_block1 = SK_FCA_Fusion_Block(nC[0], 0, shape[0], shape[0])
        # self.fusion_block2 = SK_FCA_Fusion_Block(nC[1], 1, shape[1], shape[1])
        # self.fusion_block3 = SK_FCA_Fusion_Block(nC[2], 2, shape[2], shape[2])
        # self.fusion_block4 = SK_FCA_Fusion_Block(nC[3], 3, shape[3], shape[3])
